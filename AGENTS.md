...
- `HelpView` is native WPF brief text only (the IE-based `WebBrowser` was removed: ActiveX startup risk + unthemeable chrome). Full help `HELP.html` opens in the external browser via `Process.Start(UseShellExecute)`. Its `body.dark` CSS + IE scrollbar colors are now only for external browsers (harmless to keep).
